...
def crear_item_catalogo(conexion, tabla, nombre_col, nombre_valor):
    ...
    try:
        cursor.execute(
            f"INSERT INTO {tabla} ({nombre_col}) VALUES (%s)", (nombre_valor,)
        )
        conexion.commit()
...
def actualizar_item_catalogo(
    conexion, tabla, id_col, nombre_col, id_valor, nuevo_nombre
):
    cursor = conexion.cursor()
    try:
        cursor.execute(
            f"UPDATE {tabla} SET {nombre_col} = %s WHERE {id_col} = %s",
...
def registrar_usuario_db(conexion, tipo_usuario, datos):
    ...
    try:
        ...
        sql_usuario = (
            "INSERT INTO Usuarios (Email, Password, Tipo_Usuario) VALUES (%s, %s, %s)"
        )
...
def contratar_postulante_db(conexion, id_postulacion, datos):
    ...
    try:
        ...
        cursor.execute(
            "SELECT ID_Vacante FROM Postulaciones WHERE ID_Postulacion = %s",
            (id_postulacion,),
        )
        id_vacante = cursor.fetchone()["ID_Vacante"]
        cursor.execute(
            "UPDATE Vacantes SET Estatus = 'Cerrada' WHERE ID_Vacante = %s",
            (id_vacante,),
        )
        cursor.execute(
            "UPDATE Postulaciones SET Estatus = 'Aceptada' WHERE ID_Postulacion = %s",
            (id_postulacion,),
        )
        conexion.commit()
...
def get_active_vacantes(conexion, filtro_area=None, filtro_prof=None, sort_salary=None):
    ...
    params = []
    if filtro_area:
        query += " AND ac.ID_Area_Conocimiento = %s"
        params.append(filtro_area)
    if filtro_prof:
        query += " AND p.ID_Profesion = %s"
...
def get_vacantes_por_empresa(conexion, id_empresa):
    cursor = conexion.cursor(dictionary=True)
    cursor.execute(
        "SELECT ID_Vacante, Cargo_Vacante, Descripcion_Perfil, Salario_Ofrecido, Estatus FROM Vacantes WHERE ID_Empresa = %s",
        (id_empresa,),
    )
    return cursor.fetchall()
...
def get_postulaciones_por_postulante(conexion, id_postulante):
    cursor = conexion.cursor(dictionary=True)
    cursor.execute(
        "SELECT v.Cargo_Vacante, v.Salario_Ofrecido, e.Nombre_Empresa, p.Fecha_Postulacion, p.Estatus FROM Postulaciones p JOIN Vacantes v ON p.ID_Vacante = v.ID_Vacante JOIN Empresas e ON v.ID_Empresa = e.ID_Empresa WHERE p.ID_Postulante = %s ORDER BY p.Fecha_Postulacion DESC",
        (id_postulante,),
    )
    return cursor.fetchall()
...
def get_nomina_reporte_db(conexion, id_empresa, mes, anio):
    ...
    """
    cursor.execute(query, (id_empresa, mes, anio))
    return cursor.fetchall()
...
def get_experiencias_db(conexion, id_postulante):
    cursor = conexion.cursor(dictionary=True)
    cursor.execute(
        "SELECT ID_Experiencia, Empresa, Cargo_Ocupado, Fecha_Inicio, Fecha_Fin, Descripcion FROM Experiencias_Laborales WHERE ID_Postulante = %s ORDER BY Fecha_Inicio DESC",
        (id_postulante,),
    )
    return cursor.fetchall()
...
def get_single_postulante(conexion, id_postulante):
    cursor = conexion.cursor(dictionary=True)
    cursor.execute(
        "SELECT * FROM Postulantes WHERE ID_Postulante = %s", (id_postulante,)
    )
...
def eliminar_vacante_db(conexion, id_vacante):
    ...
    try:
        cursor.execute(
            "SELECT COUNT(*) as count FROM Postulaciones WHERE ID_Vacante = %s",
            (id_vacante,),
        )
...
def eliminar_experiencia_db(conexion, id_experiencia):
    ...
    try:
        cursor.execute(
            "DELETE FROM Experiencias_Laborales WHERE ID_Experiencia = %s",
            (id_experiencia,),
        )
        conexion.commit()
