...
def contratar_postulante_db(conexion, id_postulacion, datos):
    ...
    try:
        ...
        valores = (id_postulacion, datos['Tipo_Contrato'], datos['Salario'], datos['Tipo_Sangre'], datos['Nombre Contacto Emergencia'], datos['Teléfono Contacto Emergencia'], datos['Número de Cuenta'], datos['ID_Banco'])
...
def get_active_vacantes(conexion, filtro_area=None, filtro_prof=None, sort_salary=None):
    ...
    query = """
        ...
        JOIN Areas_Conocimiento ac ON p.ID_Area_Conocimiento = ac.ID_Area_Conocimiento
...
def get_nomina_reporte_db(conexion, id_empresa, mes, anio):
    ...
    query = """
        ...
        FROM Recibos rec
        JOIN Nominas nom ON rec.ID_Nomina = nom.ID_Nomina
        JOIN Contratos c ON rec.ID_Contrato = c.ID_Contrato
        JOIN Postulaciones post ON c.ID_Postulacion = post.ID_Postulacion
        JOIN Postulantes p ON post.ID_Postulante = p.ID_Postulante
...
def get_toda_nomina_reporte_db(conexion):
    ...
    query = """
        ...
        GROUP BY e.Nombre_Empresa, nom.Mes, nom.Anio
        ORDER BY e.Nombre_Empresa, nom.Anio DESC, nom.Mes DESC
    """
    cursor.execute(query)
    return cursor.fetchall()
...
def eliminar_usuario_db(conexion, id_usuario):
    cursor = conexion.cursor()
    try:
        # La eliminación en cascada de la BD se encargará del resto
        ...
        return False, f"Error de integridad: No se puede eliminar el usuario. Revise sus contratos o postulaciones asociadas. ({e})"
    except Error as e:
